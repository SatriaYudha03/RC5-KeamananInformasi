# RC5 Email Encryption GUI - Modified for 8-bit word size (w = 8)
import base64
import smtplib
from tkinter import *
...
# Rotate left for 8-bit
def rotate_left(val, shift):
    shift %= WORD_SIZE
    return ((val << shift) | (val >> (WORD_SIZE - shift))) & 0xFF

# Rotate right for 8-bit
def rotate_right(val, shift):
    shift %= WORD_SIZE
    return ((val >> shift) | (val << (WORD_SIZE - shift))) & 0xFF

def rc5_key_schedule(key, w=8, r=1):
    b = len(key)
    u = w // 8
    c = (b + u - 1) // u
    L = [0] * c
    ...
    for _ in range(3 * max(t, c)):
        A = S[i] = rotate_left((S[i] + A + B) % MOD, 3)
        B = L[j] = rotate_left((L[j] + A + B) % MOD, (A + B) % WORD_SIZE)
        i = (i + 1) % t
        j = (j + 1) % c
    return S
...
def rc5_encrypt(block, S):
    ...
    for i in range(1, ROUNDS + 1):
        A = (rotate_left((A ^ B), B % WORD_SIZE) + S[2 * i]) % MOD
        B = (rotate_left((B ^ A), A % WORD_SIZE) + S[2 * i + 1]) % MOD
    return bytes([A, B])

def rc5_decrypt(block, S):
    A, B = block[0], block[1]
    for i in range(ROUNDS, 0, -1):
        B = rotate_right((B - S[2 * i + 1]) % MOD, A % WORD_SIZE) ^ A
        A = rotate_right((A - S[2 * i]) % MOD, B % WORD_SIZE) ^ B
    A = (A - S[0]) % MOD
    B = (B - S[1]) % MOD
    return bytes([A, B])
...
def encrypt_text(plaintext, key):
    ...
    S = rc5_key_schedule(key_bytes, w=WORD_SIZE, r=ROUNDS)
    ciphertext = b''
    for i in range(0, len(data), 2):
        ciphertext += rc5_encrypt(data[i:i+2], S)
...
def decrypt_text(ciphertext_b64, key):
    ...
    for i in range(0, len(ciphertext), 2):
        plaintext += rc5_decrypt(ciphertext[i:i+2], S)
